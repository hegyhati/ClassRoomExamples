...
set PossibleDeliveries := setof {h in Hours, o in Orders:deadline[o]>h} (h,o);
...
maximize deliveredOrders: sum{(h,o) in PossibleDeliveries} deliver[h,o];
...
for{h in Hours}
{
  for{r in Routes:select[h,r]==1}
  {
    ...
    printf "\tOrders: Totalweight: %g\n",sum{(h,o) in PossibleDeliveries:deliver[h,o]==1} weight[o];
    for{(h,o) in PossibleDeliveries:deliver[h,o]==1}
      printf "\t\tOrder %d (%s)\n",o,place[o];
  }
}
