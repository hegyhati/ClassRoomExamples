...
There is no query that can ensure that an implication is there, as the quiery cannot know about future entries.
...
So, how to check if we have a counterexample or not for an implication, for example: 
```
...
Both queries return 0 rows, meaning that there are no counterexample to the assumed implication.
The same query can be run for `nationality` which will have the same result.

To check the `date` and `raceName` based on `year` and `round` we could do the same, or do it in a single step:
...
As in many fields in IT, redundancy is the root of (if not all but a lot of) evil.
...
These three pose ever increasing rules on our schema, so if something is in 3NF, it is definitely in 2NF, and if our table violates 1NF, then it is not in 2NF either.
...
Unfortunately this will get a bit trickier. 
By definition all non-prime attributes depend on the primary key.
2NF is violated if there is a non-prime attribute that is only dependent on a real subset of the primary key (or not in 1NF).
...
Now, we have our database in 2NF, and removing those redundancies also shows on the file size:
...
There are of course plenty of variety, how these can appear, but we have this in our tables:
 - in `results`: `constructorName` -> `constructorNationality`
 - in `races`: `circuitName` -> `location`, `country`
 - in `races`: (`year`,`raceName`) -> `circuitName`

To resolve these, we do thesame as before: new tables with the dependent attributes and their keys:
...
The new schema is much more logical, is something that we would have came up with, if we were to do the design from the ground up, but getting certain information out is a bit more difficult. 
As long as we only ask questions about one table, everything is fine, e.g., what are the circuits in Germany?
...
How would you answer the question: how many times (and in which years) did Schumacher win the Italian Grand Prix?
...
We will not do that, instead jsut check whether the first 3 characters are the same, and keep silent if anybody asks about this. 
